...
(function($) {
	...
	if ($.support.msie) document.body || document.write('<body>');
	...
	$.fn.jQchart = function(cfg) {
		...
		if ($.support.msie) $(window).load(function() {
			ini()
		})
		else ini();
		...
		function drawChart(_option) {
			...
			jQuery.extend($.fn.jQchart, {
				...
				init: function(op) {
					...
					this.op.scaleXTop = this.op.scaleYBottom = this.op.height - this.op.paddingB;
				...
				mkCanvas: function(op) {
					...
					//for Opera Bug
					if (jQuery.support.opera && eval(jQuery.support.version, 10) < 9.5)
						...
						if (jQuery)
							if (jQuery.ui)
								if (jQuery.ui.draggable) {
									...
									var _cnt = 0;
									_cnt++;
									if (_cnt > 1) return; /*for fix unbind*/
				...
				wrtGraph: function(op) { //200:y=700:100
					...
					$.each(op.data, function(index, value) {
						...
						for (var i = 0; i < op.rows.length; i++) {
							...
							x += op.xGap;
							y = op.paddingT + op.chartHeight - op.rows[i] * op.chartHeight / op.dataYmax;
				...
				_debugShowPos4: function() {
					var htm = 'Properties of<br>' + '<font color="orange">$("#' + this.op.id + '").jQchart.op</font><hr>'
				...
				util: new function() {
					return {
						...
						setNextX: function(op) {
							op.begin.x = op.end.x += op.xGap;
						},
						setNextY: function(op) {
							op.begin.y = op.end.y -= op.yGap;
						}
					}
				}
